        ...
        while (true) { /*➿*/
            ...
            for (let i = 0; i < shells.length; i++) {
                ...
                if (shell.playerId !== this.playerId) {
                    this.shells.push(new Rocket(shell.id, shell.playerId, shell.x, shell.y, shell.tx, shell.ty));
                }
            }
            // Add explosions
    ...
    processInputs(delta) {
        ...
        if (keys.DOWN || keys.UP || keys.RIGHT || keys.LEFT || keys.SPACE) {
            ...
            if (keys.SPACE && ((performance.now() - this.lastFire) > Settings.rocketDelay)) {
                ...
                let ty = keys.MOUSE.y;
                this.shellIndex++;
                let shell = new Rocket(this.shellIndex.toString(), this.playerId, x, y, tx, ty);
